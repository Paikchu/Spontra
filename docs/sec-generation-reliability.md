...
模型流的辅助 I/O 单独限时：运行续期 5 秒、模型断点读写 10 秒、HTTP 错误正文读取 5 秒。辅助服务不可用时保留内存中的正文并记录故障，不能把健康的模型输出卡住；模型断点使用 ETag 条件写入，写超时后停止本次断点写入，避免迟到写覆盖新状态。底层流取消不阻塞已判定的停滞错误。上述限制不缩短持续有效输出的执行预算，也不替代最终草稿必须落盘的要求。回归包含底层取消永不返回及断点永不返回的故障注入。

## 计算框与正文一致性（2026-09-20）

对可识别的美元百万单位资本开支调节表，以当期已核验的总资本开支定位列，按数值正负号核算净资本开支；不依据 `Less` 行标题猜方向。所有匹配列和来源必须一致，列数、单位、完整性或算术不通过时不提取。其他布局保留未知，不把保守解析器当作通用表格识别器。头部展示两个算术口径及有符号调节表；算术核对通过不代表无重复计入，也不把 OCF 减净资本开支叫作公司披露的 FCF。

审核输入的组件可见性来自实际数据字段。正文、核心结论和变化表额外检查内部字段泄露、虚构已展示的第二口径，以及已核实正向调整被写成扣减的明确表述。该规则并非完整自然语言会计证明，语义审核仍保留。读者版投资含义不再沿用旧摘要的260字符硬截断。
